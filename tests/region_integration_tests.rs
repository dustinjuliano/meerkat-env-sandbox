...
use meerkat_lib::env::{Context, Symbol};
...
/// Verifies that allocating a region, creating tree structures inside it,
/// freeing it, and then allocating a new region does not result in the new
/// region inheriting stale linkages or breaking traversal via cursor moves.
#[test]
fn test_reused_block_range_stale_linkage_traversal() {
    let mut context = Context::<u32>::new();

    // 1. Allocate a parent region and some child blocks to establish linkage structure.
    let r1 = context.region_alloc(3).unwrap();
    let mut cursor = context.cursor(r1).unwrap();

    // Bind symbol in block 1
    context.bind(cursor, Symbol(10), 100);

    // Push block 2 (child of block 1)
    context.push_block(&mut cursor).unwrap();
    context.bind(cursor, Symbol(20), 200);

    // Push block 3 (child of block 2)
    context.push_block(&mut cursor).unwrap();
    context.bind(cursor, Symbol(30), 300);

    // 2. Free r1, returning blocks [1, 2, 3] to the freelist.
    context.region_free(r1);

    // 3. Allocate a new region r2 of size 3. It will reuse the same blocks [1, 2, 3] from the freelist.
    let r2 = context.region_alloc(3).unwrap();
    let mut cursor2 = context.cursor(r2).unwrap(); // Points to Block 1

    // 4. Verify that Block 1 has no stale down link.
    assert!(context.down(&mut cursor2).is_none());

    // 5. Verify that pushing a single child block 2 inside r2 works and does not inherit the stale down link to block 3.
    context.push_block(&mut cursor2).unwrap(); // Allocates Block 2 as child of Block 1. Moves cursor2 to Block 2.

    // Now verify that Block 2 has no stale down link to Block 3.
    assert!(context.down(&mut cursor2).is_none());

    // 6. Verify that we can navigate back up to Block 1.
    assert!(context.up(&mut cursor2).is_some());

    let root_cursor = context.cursor(r2).unwrap();
    // Compare inner field via debug formatting since Cursor is not directly comparable with BlockId outside the crate,
    // or just use cursor equality since Cursor implements PartialEq!
    assert_eq!(cursor2, root_cursor);
}
