...
fn test_region_reclamation_capacity() {
  ...
  assert_eq!(cap1, cap2, "Backing storage capacity should remain monotone after recycling");
}
...
fn test_arbitrary_sizes_and_allocation_splitting() {
  ...
  assert_eq!(context.blocks_capacity(), 60, "Freeing should not alter capacity");
  ...
  assert_eq!(context.blocks_capacity(), 60, "Re-allocation via splitting should not grow capacity");

  // Allocate a region of size 5
  // This should consume the remaining split slot of size 5
  let r5 = context.region_alloc(5);
  assert_eq!(context.blocks_capacity(), 60, "Re-allocation of remaining split slot should not grow capacity");

  // Allocate another region of size 5
  // Since the freelist is now empty, capacity must grow to 65
  let r6 = context.region_alloc(5);
  assert_eq!(context.blocks_capacity(), 65, "Capacity should grow when freelist cannot satisfy the request");
...
fn test_zero_sized_allocations() {
  ...
  assert_eq!(context.blocks_capacity(), 0, "Zero-sized allocation should not resize the blocks array");
  ...
  // Allocate size 10
  let r2 = context.region_alloc(10);
  assert_eq!(context.blocks_capacity(), 10, "Capacity should be exactly 10 after allocating size 10");
...
fn test_first_fit_strategy() {
  ...
  // Free r1, r2, r3. Freelist now has free slots: [10, 30, 20] in
  // that order
  context.region_free(r1);
  context.region_free(r2);
  context.region_free(r3);
  ...
  assert_eq!(context.blocks_capacity(), 62, "Size 20 slot should be intact and used without growing capacity");
...
fn test_exact_vs_overflow_fit() {
  ...
  // Request size 10. It should fit perfectly in the original size 10 free slot
  let r3 = context.region_alloc(10);
  assert_eq!(context.blocks_capacity(), 21, "Size 10 request should reuse the original free slot");
...
fn test_large_freelist_split() {
  ...
  // Request size 1. This should split the size 100 slot, leaving
  // 99 blocks in the freelist
...
fn test_alternating_growth_and_reuse() {
  ...
  // Request size 100
  // The freelist slot of 50 is too small. It must allocate 100
  // new blocks
...
fn test_high_volume_stress_cycle() {
  ...
  // Allocate 100 regions of size 1. This should split the 10
  // free slots of size 10 into 100 slots of size 1
