...
fn test_cursor_traversal() {
    ...
    context.bind(cursor, Symbol(1), 10);
    ...
    context.bind(cursor, Symbol(2), 20);
    ...
    // Push block 3
    context.push_block(&mut cursor).unwrap();
    context.bind(cursor, Symbol(3), 30);
    ...
    assert_eq!(context.find(cursor, Symbol(2)), Some(&20));
    assert_eq!(context.find(cursor, Symbol(3)), None);

    // Move next to sibling (3)
    context.next(&mut cursor).unwrap();
    assert_eq!(context.find(cursor, Symbol(3)), Some(&30));
    assert_eq!(context.find(cursor, Symbol(2)), None);
...
fn test_push_region_and_lexical_continuity() {
    ...
    context.bind(cursor, Symbol(10), 1000);

    context.push_block(&mut cursor).unwrap(); // Block 2 in region 0
    context.bind(cursor, Symbol(20), 2000);
    ...
    context.bind(cursor, Symbol(30), 3000);

    // Symbol resolution from nested region should climb boundaries to parent region
    assert_eq!(context.find(cursor, Symbol(30)), Some(&3000));
    assert_eq!(context.find(cursor, Symbol(20)), Some(&2000));
    assert_eq!(context.find(cursor, Symbol(10)), Some(&1000));
    assert_eq!(context.find(cursor, Symbol(99)), None);
...
fn test_invalid_cursor_handling() {
    ...
    context_mut.bind(invalid_cursor, Symbol(1), 1);
}
...
fn test_region_dynamic_growth_and_coalescing_integration() {
    ...
    for i in 2..=10 {
        context.push_block(&mut cursor).unwrap();
        context.bind(cursor, Symbol(i), i * 10);
    }

    // Verify that all sequential scopes can resolve their respective bindings
    for i in 1..=10 {
        assert_eq!(context.find(cursor, Symbol(i)), Some(&(i * 10)));
    }
...
fn test_multi_level_region_resolution() {
    ...
    assert_eq!(context.find(cursor, Symbol(3)), Some(&30));
    assert_eq!(context.find(cursor, Symbol(2)), Some(&20));
    assert_eq!(context.find(cursor, Symbol(1)), Some(&10));
}
...
fn test_shadowing_across_region_boundaries() {
    ...
    context.bind(cursor, Symbol(42), 200);

    // Shadowed value visible in inner region
    assert_eq!(context.find(cursor, Symbol(42)), Some(&200));
...
/// Verifies stale cursor safety: after freeing a region, calling `find`
/// with a stale cursor fires the membership `debug_assert!`, which catches
/// the programming error of retaining a cursor across `region_free`
#[test]
#[should_panic(expected = "find: cursor points to a block not live in its region")]
fn test_stale_cursor_behavior_after_region_free() {
    ...
    // Using the stale cursor fires the debug_assert! in find
    context.find(cursor, Symbol(42));
}
...
fn test_lexical_tree_climbing_branch_isolation() {
    ...
    let r = context.region_alloc(5).unwrap();
    let mut cursor = context.cursor(r).unwrap(); // Block 1
    context.bind(cursor, Symbol(10), 100);

    // Branch A: Push child block 2
    context.push_block(&mut cursor).unwrap(); // Block 2
    context.bind(cursor, Symbol(20), 200);
    ...
    context.bind(cursor, Symbol(30), 300);

    // Query from Block 3: should resolve parent Symbol(10) and local Symbol(30), but NOT sibling Symbol(20)
    assert_eq!(context.find(cursor, Symbol(30)), Some(&300));
    assert_eq!(context.find(cursor, Symbol(10)), Some(&100));
...
fn test_climbing_past_empty_scopes() {
    ...
    context.bind(cursor, Symbol(40), 400);

    // Query from Block 4: should find Symbol(10) in Block 1 and Symbol(40) in Block 4
    assert_eq!(context.find(cursor, Symbol(40)), Some(&400));
    assert_eq!(context.find(cursor, Symbol(10)), Some(&100));
}
...
fn test_region_id_recycling_isolation() {
    ...
    context.bind(cursor_a, Symbol(42), 100);
...
fn test_multiple_coexisting_cursors() {
    ...
    context.bind(cursor_a, Symbol(1), 10);
    ...
    assert_eq!(context.find(cursor_b_moved, Symbol(1)), Some(&10));
}
...
fn test_shadowing_independence_in_sibling_scopes() {
    ...
    let r = context.region_alloc(5).unwrap();
    let mut cursor = context.cursor(r).unwrap(); // Parent Block 1
    context.bind(cursor, Symbol(42), 10);

    // Sibling A: Pushes child, shadows Symbol(42) to 20
    context.push_block(&mut cursor).unwrap(); // Block 2
    context.bind(cursor, Symbol(42), 20);

    context.up(&mut cursor).unwrap(); // Back to parent

    // Sibling B: Pushes child, shadows Symbol(42) to 30
    context.push_block(&mut cursor).unwrap(); // Block 3
    context.bind(cursor, Symbol(42), 30);

    // Cursor at Block 3 resolves Symbol(42) to 30
    assert_eq!(context.find(cursor, Symbol(42)), Some(&30));

    // Cursor at Block 2 resolves Symbol(42) to 20
    context.up(&mut cursor).unwrap();
    context.down(&mut cursor).unwrap();
    assert_eq!(context.find(cursor, Symbol(42)), Some(&20));
}
...
fn test_symbol_re_binding_in_same_block() {
    ...
    context.bind(cursor, Symbol(1), 20);
    assert_eq!(context.find(cursor, Symbol(1)), Some(&20));
}
...
fn test_symbol_lookup_in_non_contiguous_region() {
    ...
    context.push_block(&mut cursor_a).unwrap();
    context.bind(cursor_a, Symbol(2), 20);
...
/// Verifies that using a stale cursor for lookup after `region_free` fires
/// the membership `debug_assert!`. Navigation (up/down/next) is intentionally
/// not guarded because the block arena links are preserved and traversal is
/// cheap; only operations that act on region-owned state (`find`, `bind`,
/// `push_block`) carry the full membership check
#[test]
#[should_panic(expected = "find: cursor points to a block not live in its region")]
fn test_stale_cursor_navigation_vs_lookup() {
    ...
    // Traversals still succeed: block linkages in the arena are preserved
    context.up(&mut cursor).unwrap(); // Go back to Block 1

    // Lookup on Block 1 fires the debug_assert! because the cursor is stale
    context.find(cursor, Symbol(1));
}
